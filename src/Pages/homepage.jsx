...
function Homepage() {
  ...
  const contentMap = {
    lvcc: {
      ...
      description: `Leo sit amet facilisis tristique, nunc leo cursus elit, nec placerat nisl sem vitae nunc. Sed id magna nec libero feugiat aliquam. Curabitur et arcu vel turpis convallis facilisis. Aenean ac malesuada odio, et faucibus augue. Vivamus vel nulla et purus consequat fringilla. Ut aliquam, leo sit amet facilisis tristique, nunc leo cursus elit, nec placerat nisl sem vitae nunc. Sed id magna nec libero feugiat aliquam. Curabitur et arcu vel turpis convallis facilisis.`
    },
    bsis: {
  ...
  return (
    <div className="relative w-full min-h-screen bg-white">
      <header className={`w-full top-0 z-50 p-5 fixed transition-colors duration-300 ${scrolled ? 'bg-white shadow-md' : 'bg-transparent'}`}>
      ...
      {/* Home Section */}
      <div id="home">
        <img src="pictures/LVBG.jpg" alt="" className="absolute opacity-20 w-full h-[1000px]" />
      </div>

      <div className="relative flex items-center justify-between px-20 pt-52 pb-10 bg-cover bg-center h-[1000px]">
        <div className="max-w-2xl text-left ml-10">
          ...
          <button className="mt-6 px-15 py-3 bg-[#1E3A8A] text-white font-semibold text-[24px] rounded-lg" onClick={Explore}>
            Explore
          </button>
        </div>
      </div>
      ...
      {/* About Section */}
      <div id="about" className="flex justify-around py-6 pt-15 bg-transparent cursor-pointer">
        <div className="text-center" onClick={() => setSelected('lvcc')}>
          <img
            src="pictures/logo.png"
            alt="LVCC"
            className={`h-27 mx-auto transition duration-300 ${selected !== 'lvcc' ? 'grayscale' : ''}`}
          />
          <p className={`text-[23px] ${selected !== 'lvcc' ? 'text-gray-500' : 'text-black'}`}>
            LVCC
          </p>
        </div>
        ...
        <div className="text-center" onClick={() => setSelected('bsis')}>
          <img
            ...
            alt="BSIS"
            className={`h-27 mx-auto transition duration-300 ${selected !== 'bsis' ? 'grayscale' : ''}`}
          />
          <p className={`text-[23px] ${selected !== 'bsis' ? 'text-gray-500' : 'text-black'}`}>
            BSIS
          </p>
        </div>
        ...
        <div className="text-center" onClick={() => setSelected('aim')}>
          <img
            ...
            alt="AIM"
            className={`h-27 mx-auto transition duration-300 ${selected !== 'aim' ? 'grayscale' : ''}`}
          />
          <p className={`text-[23px] ${selected !== 'aim' ? 'text-gray-500' : 'text-black'}`}>
            AIM
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-center items-center py-12 bg-white">
        ...
        <div className="max-w-[850px] pl-8">
          <div className="flex items-center mb-15 ">
            <img src={contentMap[selected].logo} alt={contentMap[selected].title} className="h-[126px] mr-4" />
            <h2 className="text-[35px] font-bold text-[#2D0F7F]">{contentMap[selected].title}</h2>
          </div>
