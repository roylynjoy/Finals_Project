...
function RecentlyAccessedRole({ loading }) {
  ...
  return (
    ...
    >
      <h2 className="text-[27px] text-center font-semibold text-[#3F3F46] px-2 mb-5 z-10">
        Recently Accessed Role
      </h2>

      <div
        className="relative flex-1 w-full rounded-[10px] overflow-hidden flex items-center justify-center"
        onClick={handleClick}
      >
        {/* Background */}
        <div
        ...
        {/* Dim Layer */}
        <div className="border-2 border-[#B9B9B9] rounded-[10px] absolute inset-0 bg-black/5 z-10 transition-opacity duration-500 ease-in-out" />

        {/* Hover Overlay */}
        <div className="absolute border-2 border-[#005CFA] rounded-[10px] inset-0 z-20 opacity-0 group-hover:opacity-100 bg-white/70 backdrop-blur-sm flex items-center justify-center transition-opacity duration-300 ease-in-out">
          <span className="text-[24px] text-black/70">View Recent</span>
        </div>

        {/* Title Text */}
        <div className="relative z-30 text-center px-4 transition-opacity duration-500 ease-in-out group-hover:opacity-0">
          <span
